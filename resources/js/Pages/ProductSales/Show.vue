...
function openRebate(item) {
    ...
    rebateForm.from_page = 'ProductSaleDetail';
...
function submitRebate() {
    rebateForm.post(route('inertia.product-sales.rebate'), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <div class="container-fluid">
        <PageTitle title="Product Management" :crumbs="['Home', 'ProductManagement', 'Sales', 'Detail']" />
        <div class="card">
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-6"><h4>Product Sale Detail</h4></div>
                    <div class="col-6 text-end">
                        <a :href="route('inertia.product-sales.invoice', sale)" class="btn btn-info btn-sm" target="_blank">
                            <i class="fa fa-file"></i> Invoice
                        </a>
                        <Link :href="route('inertia.product-sales.index')" class="btn btn-secondary btn-sm ms-1">
                            <i class="fa fa-arrow-left"></i> Product Sales
                        </Link>
                    </div>
                </div>
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in sale.items" :key="item.id">
                            ...
                            <td>
                                <button
                                    v-if="can('productSales.update') && Number(item.product_total_qty) > 0"
                                    type="button"
