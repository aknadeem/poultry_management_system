...
class BrokerBalanceController extends Controller
{
    ...
    {
        return view('partymanagement.brokers.brokerbalance.index');
    }
    ...
    public function getbrokersBalanceList()
    {
        $balances = BrokerBalance::with('broker:id,name,guardian_name,cnic_no,contact_no')->orderBy('id','DESC')->withCasts([
            'created_at' => 'date:d M, Y'
        ])->get();
        return DataTables::of($balances)
            ->addIndexColumn()
            ->addColumn('broker_id', function($row){
                return '<span> '.$row?->broker?->name.' </span>';
            })
